...
import json
import hopp.to_organize.probably_to_project
...
def hopp_for_h2(site, scenario, technologies, wind_size_mw, solar_size_mw, storage_size_mw, storage_size_mwh, storage_hours,
    ...
    if 'wind' in technologies:
        ...
        if custom_powercurve:
            project_path = os.path.abspath(os.path.join(os.path.dirname(hopp.to_organize.__file__), "probably_to_project", "H2_Analysis"))
            powercurve_file = open(os.path.join(project_path, scenario['Powercurve File']))
            ...
            if powercurve_file_extension == '.csv':
                curve_data = pd.read_csv(os.path.join(project_path, scenario['Powercurve File']))
