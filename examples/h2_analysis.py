...
sample_site['lat'] = lat
sample_site['lon'] = lon
tower_height_list = [120]  # [80, 90, 100, 110, 120, 130, 140]
                ...
                for itc_avail in itc_avail_list:
                    for tower_height in tower_height_list:
                        ...
                        storage_size_mwh, result['outputs']['Scenario']['Site']['Wind']['lcoe_us_dollars_per_kwh'],
                                            ...
                                            lcoe,
                                            feedstock_cost_h2, np.sum(combined_pv_wind_storage_power_production), np.sum(energy_shortfall), np.sum(combined_pv_wind_curtailment))
                        ...
                        # plt.show()
                        plt.savefig("wind_pv_plainview_production_{}.png".format(critical_load_pct), dpi=240,
                        ...
                        fig2.tight_layout()
                        # fig2.show()
                        fig2.savefig("wind_pv_plainview_shortfall_curtailment_{}.png".format(critical_load_pct),
                                     dpi=240, bbox_to_inches="tight")
                        plt.close('all')
