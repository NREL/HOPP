...
def test_detailed_battery_dispatch(site):
    ...
    # results = HybridDispatchManager.mindtpy_solve_call(model)
...
def test_pv_wind_battery_hybrid_dispatch(site):
    ...
    hybrid_plant.dispatch_manager.dispatch.initialize_parameters()
    hybrid_plant.dispatch_manager.dispatch.update_time_series_parameters(0)
...
def test_hybrid_dispatch_constant_output(site):
    ...
    HybridDispatchManager._glpk_solve_call(hybrid_plant.dispatch_manager.pyomo_model)
    ...
    assert max(battery_gen) == pytest.approx(10, rel=1e-2)
