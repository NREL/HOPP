...
def run_hopp_calc(Site, scenario_description, bos_details, total_hybrid_plant_capacity_mw, solar_size_mw, wind_size_mw,
    ...
    # Change system coordinates before running:
    Nx = 8
    Ny = 5
    ...
    # defaults_ideal['Wind']['Windpower']['Farm']['wind_farm_xCoordinates'] = list(turbX)
    # defaults_ideal['Wind']['Windpower']['Farm']['wind_farm_yCoordinates'] = list(turbY)
...
def run_hybrid_calc(year, site_num, scenario_descriptions, results_dir, load_resource_from_file,
    ...
    """
    print("Running HOPP calc for site number: {}".format(site_num))
...
if __name__ == '__main__':
    ...
    year = 2013
    # N_lat = 50  # number of data points
    # N_lon = 95
    N_lat = 50  # number of data points
    N_lon = 95
    # desired_lats = 35.21
    ...
    # desired_lats = np.linspace(33.833504, 49.3556, N_lat)
    # desired_lons = np.linspace(-129.22923, -65.7146, N_lon)
    ...
    if load_resource_from_file:
        ...
        #  Form the site_details Dataframe manually in lieu of using resource_loader_file method
        if os.path.exists('filtered_site_details.csv'):
            site_details = pd.read_csv('filtered_site_details.csv')
        else:
            ...
            site_details['wind_filenames'] = [''] * len(site_nums)
            site_details['solar_filenames'] = [''] * len(site_nums)
            site_details['year'] = [str(year)] * len(site_nums)
            site_details = filter_sites(site_details, location='usa only')
    ...
    # Nx = 1
    # Ny = 200
    # turbine_x = np.linspace(0, Nx * spacing, Nx)
    # turbine_y = np.linspace(0, Ny * spacing, Ny)
    # turbX = np.zeros(Nx * Ny)
    # turbY = np.zeros(Nx * Ny)
    # count = 0
    # for i in range(Nx):
