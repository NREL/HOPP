...
default_options = HybridDispatchOptions()
...
def test_simple_battery_dispatch():
    ...
    battery_dispatch = np.array(battery.dispatch.power)
    battery_actual = np.array(battery.generation_profile[0:dispatch_n_look_ahead]) * 1e-3   # convert to MWh
    battery_sl_dispatch = np.array(battery_sl.dispatch.power)
    battery_sl_actual = np.array(battery_sl.generation_profile) * 1e-3   # convert to MWh
