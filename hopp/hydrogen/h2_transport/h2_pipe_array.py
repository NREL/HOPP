...
def run_pipe_analysis(L,m_dot,p_inlet,p_outlet,depth, data_location="data_tables/"):
    '''
    ...
    yield_strengths = pd.read_csv(data_location+'steel_mechanical_props.csv',index_col = None,header = 0)
    yield_strengths = yield_strengths.loc[~yield_strengths['Grade'].isin(['A','B','A25'])].reset_index()
    schedules_all = pd.read_csv(data_location+'pipe_dimensions_metric.csv',index_col = None,header = 0)
