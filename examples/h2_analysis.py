...
# Get resource
site_name = 'Seven Mile Hill - Wyoming' #'Plainview Bioenergy - Texas'
lat = 35.21 #41.939 #46.1  #flatirons_site['lat']
lon = -101 #-106.37 #-94.33  #flatirons_site
year = 2013
...
#TODO Remove after 3 bar analysis
three_scenario = True
scenario_choice = 'No Help'  # 'No Help' 'Policy Help' 'Policy Help + Aggressive Tech Innovation'
            ...
            for ptc_avail in ptc_avail_list:
                for itc_avail in itc_avail_list:
                    for tower_height in tower_height_list:
                        ...
                        if three_scenario:
                            if scenario_choice == 'No Help':
                                ptc_avail = True
                                itc_avail = True
                                tower_height = 140
                                useful_life = 35
                                debt_equity_split = 90
                            elif scenario_choice == 'Policy Help':
                                ptc_avail = True
                                itc_avail = True
                                tower_height = 100
                                useful_life = 25
                                debt_equity_split = 90
                            elif scenario_choice == 'Policy Help + Aggressive Tech Innovation':
                                ptc_avail = True
                                itc_avail = True
                                useful_life = 35
                                debt_equity_split = 90
                            ...
                            feedstock_cost_h2 = lcoe * 55.5/100  # $/kg
                            ...
                            feedstock_cost_h2_via_net_cap_cost_lifetime_h2 = hybrid_plant.grid.financial_model.Outputs.adjusted_installed_cost /\
                                                                                  ((kw_continuous/55.5)*(8760 * useful_life))
                            feedstock_cost_h2_via_net_cap_cost_lifetime_h2_reopt = result['outputs']['Scenario']['Site']['Financial']['net_capital_costs'] / ((kw_continuous/55.5)*(8760*useful_life))
                            ...
                            storage_size_mwh, result['outputs']['Scenario']['Site']['Wind']['lcoe_us_dollars_per_kwh'],
                                                ...
                                                lcoe,
                                                feedstock_cost_h2_via_net_cap_cost_lifetime_h2, np.sum(combined_pv_wind_storage_power_production), np.sum(energy_shortfall), np.sum(combined_pv_wind_curtailment))
                            ...
                            save_outputs_loop['H2 Elec Feedstock Cost/kW'].append(feedstock_cost_h2_via_net_cap_cost_lifetime_h2)
