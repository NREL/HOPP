...
financial_summary_outputs  = pd.read_sql_query("SELECT * From Summary",conn)
...
for i in financial_summary_outputs.index:
    
    #i=0
    # Scenario of Interest    
    # grid_case = RODeO_summary_outputs_optstorage.iloc[i, 127]
    # storage_case = RODeO_summary_outputs_optstorage.iloc[i, 126]
    hydrogen_model_case = financial_summary_outputs.loc[i,'Hydrogen model']
    location_case = financial_summary_outputs.loc[i, 'Site']
    policy_case = financial_summary_outputs.loc[i,'Policy Option'].replace(' ', '-')
    ...
    if hydrogen_model_case == 'PyFAST':
        hydrogen_scenario = financial_summary_outputs[['ATB Year','Site','Hydrogen model','Policy Option','LCOH: Hydrogen Storage ($/kg)','LCOH: Compression ($/kg)','LCOH: Electrolyzer CAPEX ($/kg)','LCOH: Desalination CAPEX ($/kg)',
    ...
    steel_scenario = financial_summary_outputs[['ATB Year','Site','Hydrogen model','Policy Option','Steel price: EAF and Casting CAPEX ($/tonne)','Steel price: Shaft Furnace CAPEX ($/tonne)','Steel price: Oxygen Supply CAPEX ($/tonne)',
