...
class PEM_H2_Clusters:
    ...
    Parameters
    _____________
    numpy array: P_input_external_kW
        1-D array of time-series external power supply

    int: cluster_size_mw
    int: plant_life
        plant life in years
    bool: include_degradation_penalty
    dictionary: input_dictionary
        user_defined_EOL_percent_eff_loss: bool (default=False)
            eol_eff_percent_loss: float (default=13) CHECK: maybe 0.13
        user_defined_eff: bool (default=False)
            rated_eff_kWh_pr_kg: float (default=[]), not currently implemented
        reset_uptime_deg_to_target: bool (default=True)
            steady_state_stack_durability_hours: int (default=80000)
            steady_state_stack_durability_reference_capacity_factor: float (default=0.97)
        user_set_max_h2_limit: bool
            max_cluster_h2_lim: int

        use_onoff_deg: bool
        use_uptime_deg: bool
        use_fatigue_deg: bool
    ...
    def __init__(self, cluster_size_mw, plant_life, user_defined_EOL_percent_eff_loss, eol_eff_percent_loss=[],user_defined_eff = False,rated_eff_kWh_pr_kg=[],include_degradation_penalty=True,dt=3600):
        ...
        self.key_system_parameters={}
        self.degradation_tracker={}
        self.system_hourly_performance_tracker={}
        self.stack_hourly_performance_tracker={}
        self.simulation_performance_summary={}
        self.system_hourly_performance_tracker['main']={}

        self.user_set_max_h2_limit=False # TODO: add as input
        self.kg_h2_saturation_lim=[]
    ...
    def get_dynamic_h2_multiplier(self):
    ...
    def get_power_current_voltage_per_stack(self,cluster_input_power_kw):
        self.stack_hourly_performance_tracker['PIV-Info']={}
        self.system_hourly_performance_tracker['PIV-Details']={}
        self.system_hourly_performance_tracker['PIV-Details']['Initial Power Curtailed']=self.output_dict['Curtailed Power [kWh]'] 
        #self.system_hourly_performance_tracker={}
        power_per_stack = np.where(self.n_stacks_op>0,cluster_input_power_kw/self.n_stacks_op,0)
        stack_current =calc_current((power_per_stack,self.T_C), *self.curve_coeff)
        self.stack_hourly_performance_tracker['PIV-Info']['Stack Current [init]']=stack_current
        if self.include_deg_penalty:
            ...
            self.stack_hourly_performance_tracker['PIV-Info']['Cell Voltage [init]']=V_init
            self.stack_hourly_performance_tracker['PIV-Info']['Voltage Degradation [init]']=deg_signal
            self.stack_hourly_performance_tracker['PIV-Info']['Degraded Cell Voltage']=V_cell_deg
            ...
            self.stack_hourly_performance_tracker['PIV-Info']['Stack Current [corrected]']=stack_current
            V_cell_equiv = self.cell_design(self.T_C,stack_current)
            self.stack_hourly_performance_tracker['PIV-Info']['Cell Voltage [corrected]']=V_cell_equiv
            V_cell = V_cell_equiv + deg_signal
            self.stack_hourly_performance_tracker['PIV-Info']['Degraded Cell Voltage [corrected]']=V_cell
            Vdeg=deg_signal
        else:
            V_init=self.cell_design(self.T_C,stack_current)
            V_ignore,deg_signal=self.full_degradation(V_init)
            V_deg=np.zeros(len(V_init))
            V_cell=self.cell_design(self.T_C,stack_current) #+self.total_Vdeg_per_hr_sys
        if self.user_set_max_h2_limit:
            stack_current,power_curtailed_kW=self.set_max_current_limit(self.kg_h2_saturation_lim,stack_current,Vdeg,cluster_input_power_kw)
            self.stack_hourly_performance_tracker['PIV-Info']['Stack Current [saturated]']=stack_current
            self.system_hourly_performance_tracker['PIV-Details']['Cluster Power Curtailed from Saturation']=power_curtailed_kW
            
        stack_power_consumed = (stack_current * V_cell * self.N_cells)/1000
        system_power_consumed = self.n_stacks_op*stack_power_consumed
        self.stack_hourly_performance_tracker['PIV-Info']['Stack Power Consumed [kW][final]']=stack_power_consumed 
        self.system_hourly_performance_tracker['main']['System Power Consumed [kWh]']=system_power_consumed
    ...
    def run(self,input_external_power_kw):
        ...
        self.system_hourly_performance_tracker['main']['Cluster Status']=self.cluster_status
        ...
        # cluster_cycling = np.array(cluster_cycling)
        h2_multiplier=self.get_dynamic_h2_multiplier()
        #h2_multiplier = np.where(cluster_cycling > 0, startup_ratio, 1)
