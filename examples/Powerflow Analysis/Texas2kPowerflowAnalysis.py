...
#### Powerflow analysis of hurricane Dolly event - Texas #####
#TODO:
# - Access each column individually to plot.
# - Sample plot just for the first node
# - Load the branch configuration details
# - Load the bus configuration details
# - Create node connection network, label points
# -

### Making GIF from saved results
images = []

# Import load P and Q (real and reactive) timeseries for each node at 5 minute resolution
df_P = pd.read_csv('tamu2k_grid/base_case_loadP_df.csv')
df_Q = pd.read_csv('tamu2k_grid/base_case_loadQ_df.csv')

# Plot sample of real power
fig_1, ax_1 = plt.subplots(figsize=(15, 10))
ax_1.plot(df_P['1001'])
ax_1.set_xlabel("Date/Time")
ax_1.set_ylabel("Real Power (kW)")
ax_1.grid(alpha=0.7)
ax_1.grid(alpha=0.2, which="minor")
titletext = 'Texas 2k Node - Hurricane Dollar Scenario \n' \
            'Real Power (kW) \n'\
            'Node: {}'\
    .format('1001')
ax_1.set_title(titletext)

# Import Bus Information
df_Bus = pd.read_csv('tamu2k_grid/bus.csv')
bus_IDs = df_Bus['bus_i']

# Import the branch configuration details (from and to connections)
df_Branch = pd.read_csv('tamu2k_grid/branch.csv')
df_from_bus = df_Branch['fbus']
df_to_bus = df_Branch['tbus']

# Create the node connection network
