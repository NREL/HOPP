...
from hybrid.dispatch import HybridDispatch, HybridDispatchOptions, DispatchProblemState, SolverOptions
...
class HybridDispatchManager:
    ...
    def __init__(self,
        ...
        if self.options.objective == 'follow_schedule':
            self.dispatch.create_min_operating_cost_objective()
        elif self.options.objective == 'constant_output':
            self.dispatch.create_constant_output_objective()
    ...
    def solve_dispatch_model(self, start_time: int, n_days: int):
        ...
        if self.options.solver == 'glpk':
            solver_results = self._glpk_solve()
        elif self.options.solver == 'cbc':
            solver_results = self._cbc_solve()
        elif self.options.solver == 'xpress':
            solver_results = self._xpress_solve()
        elif self.options.solver == 'xpress_persistent':
            solver_results = self._xpress_persistent_solve()
        elif self.options.solver == 'gurobi_ampl':
            solver_results = self._gurobi_ampl_solve()
        elif self.options.solver == 'gurobi':
            solver_results = self._gurobi_solve()
        else:
    ...
    def _glpk_solve_call(pyomo_model: pyomo.ConcreteModel,
        ...
        HybridDispatchManager._log_and_solution_check(log_name, solver_options.instance_log, results.solver.termination_condition, pyomo_model)
        return results
        
    def _glpk_solve(self):
        return HybridDispatchManager._glpk_solve_call(self.pyomo_model,
                                                           self.options.log_name,
                                                           self.options.solver_options)
        
    @staticmethod
    def _gurobi_ampl_solve_call(pyomo_model: pyomo.ConcreteModel,
                               log_name: str = "",
                               user_solver_options: dict = None):
    ...
    def _gurobi_ampl_solve(self):
        return HybridDispatchManager._gurobi_ampl_solve_call(self.pyomo_model,
                                                                  self.options.log_name,
                                                                  self.options.solver_options)
                                                                  
    @staticmethod
    def _gurobi_solve_call(opt: pyomo.SolverFactory,
    ...
    def _gurobi_solve(self):
        ...
        return HybridDispatchManager._gurobi_solve_call(self.opt,
                                                             self.pyomo_model,
                                                             self.options.log_name,
                                                             self.options.solver_options)

    @staticmethod
    def _cbc_solve_call(pyomo_model: pyomo.ConcreteModel,
                       log_name: str = "",
                       user_solver_options: dict = None):
    ...
    def _cbc_solve(self):
        return HybridDispatchManager._cbc_solve_call(self.pyomo_model,
                                                          self.options.log_name,
                                                          self.options.solver_options)

    @staticmethod
    def _xpress_solve_call(pyomo_model: pyomo.ConcreteModel,
                          log_name: str = "",
                          user_solver_options: dict = None):
    ...
    def _xpress_solve(self):
        return HybridDispatchManager._xpress_solve_call(self.pyomo_model,
                                                             self.options.log_name,
                                                             self.options.solver_options)

    @staticmethod
    def _xpress_persistent_solve_call(opt: pyomo.SolverFactory,
                                     pyomo_model: pyomo.ConcreteModel,
                                     log_name: str = "",
                                     user_solver_options: dict = None):
    ...
    def _xpress_persistent_solve(self):
        if self.opt is None:
            self.opt = pyomo.SolverFactory('xpress', solver_io='persistent')

        return HybridDispatchManager._xpress_persistent_solve_call(self.opt,
                                                                        self.pyomo_model,
                                                                        self.options.log_name,
                                                                        self.options.solver_options)
    @staticmethod
    def _mindtpy_solve_call(pyomo_model: pyomo.ConcreteModel,
                           log_name: str = ""):
        ...
        HybridDispatchManager._log_and_solution_check("", "", results.solver.termination_condition, pyomo_model)
        return results

    @staticmethod
    def _log_and_solution_check(log_name:str, solve_log: str, solver_termination_condition, pyomo_model):
        if log_name != "":
            HybridDispatchManager.append_solve_to_log(log_name, solve_log)
        HybridDispatchManager._check_solve_condition(solver_termination_condition, pyomo_model)

    @staticmethod
    def _check_solve_condition(solver_termination_condition, pyomo_model):
        if solver_termination_condition == TerminationCondition.infeasible:
            HybridDispatchManager._print_infeasible_problem(pyomo_model)
    ...
    @staticmethod
    def _append_solve_to_log(log_name: str, solve_log: str):
    ...
    @staticmethod
    def _display_all_blocks(model: pyomo.ConcreteModel):
