...
def get_inputs(turbine_model="osw_18MW", verbose=False, show_plots=False, save_plots=False):
    ...
    if use_floris: #TODO replace elements of the file
        ...
        floris_file = floris_dir + "floris_input15MW_base" + ".yaml"
...
def run_orbit(plant_config, verbose=False, weather=None):
    ...
    if verbose:
        ...
        print("Export cable material CAPEX: %.2f M USD" %(project.phases["ElectricalDesign"].total_cable_length_by_type['HVDC_2000mm_275kV']*430000*1E-6))
        print("\n")
...
def run_lcoh(plant_config, expenditures_annual, electrolyzer_physics_results, verbose=False):
    ...
    tlcc_total_costs = npf.npv(plant_config["finance_parameters"]["discount_rate"], expenditures_annual)
    ...
    H2_Results = electrolyzer_physics_results["H2_Results"]
    discounted_h2_production = npf.npv(plant_config["finance_parameters"]["discount_rate"], [H2_Results['hydrogen_annual_output']]*30)
   # print('discounted h2 production',discounted_h2_production)
    ...
    if verbose:
        print("\nLCOH: ", "%.2f" % (LCOH_cf_method_total_hvdc), "$/kg")

    return LCOH_cf_method_total_hvdc
...
def run_profast_grid_only(plant_config, orbit_project, electrolyzer_physics_results, capex_breakdown, opex_breakdown, hopp_results, design_scenario, verbose=False, show_plots=False, save_plots=False):
    ...
    # # energy_purchase[0] = energy_purchase[1] = 0.0
    # print("shortfall ", annual_energy_shortfall*1E-3/(365*24))
    # print("cost per year: ", energy_purchase*plant_config["project_parameters"]["ppa_price"])
    ...
    # used fixed cost unless tied to unit of commodity (fixed costs is $/yr, feedstock is $/unit commodity)
    # capacity only in units per day
    ...
    # all_flow = pf.plot_costs()

    # a = all_flow.loc[all_flow['Type'].isin(['Operating Revenue','Financing cash inflow']),'NPV'].sum()

    # b = all_flow.loc[all_flow['Type'].isin(['Operating Expenses','Financing cash outflow']),'NPV'].sum()

    # c = all_flow.loc[all_flow['Name']=='Hydrogen sales','NPV'].sum()
    ...
    if False:
        ...
        pf.plot_costs_yearly(per_kg=False, scale='M', remove_zeros=True, remove_depreciation=False, fileout="annual_cash_flow_grid_only.pdf")

        pf.plot_costs_yearly2(per_kg=False, scale='M', remove_zeros=True, remove_depreciation=False, fileout="annual_cash_flow_grid_only.html")
        pf.plot_capital_expenses(fileout="capital_expense_grid_only.pdf")
        pf.plot_cashflow(fileout="cash_flow_grid_only.pdf")
...
def run_profast_full_plant_model(plant_config, orbit_project, electrolyzer_physics_results, capex_breakdown, opex_breakdown, hopp_results, incentive_option, design_scenario, verbose=False, show_plots=False, save_plots=False):
    ...
    # print("h2 kg/day", electrolyzer_physics_results["H2_Results"]['hydrogen_annual_output']/365.0)
    # print("H2 Produced: ", electrolyzer_physics_results["H2_Results"]['hydrogen_annual_output'])
    pf = ProFAST.ProFAST('blank')
    pf.set_params('commodity', {"name":'Hydrogen',"unit":"kg","initial price":100,"escalation": gen_inflation})
    pf.set_params('capacity', electrolyzer_physics_results["H2_Results"]['hydrogen_annual_output']/365.0) #kg/day
    pf.set_params('maintenance',{"value":0,"escalation": gen_inflation})
    ...
    if verbose:
        ...
        # IRR has multiple roots, should return multiple roots - or give initial guess of discount rate
        # profitability index should 1.0 at the discount rate (also has issues though)
...
def post_process_simulation(pf_lcoh, pf_lcoe, hopp_results, electrolyzer_physics_results, plant_config, h2_storage_results, capex_breakdown, opex_breakdown, orbit_project, platform_results, desal_results, design_scenario, plant_design_number, incentive_option, solver_results=[], show_plots=False, save_plots=False):#, lcoe, lcoh, lcoh_with_grid, lcoh_grid_only):
    ...
    print("\nH2 Storage CAPEX $/kg capacity: ", capex_breakdown["h2_storage"]/h2_storage_results["h2_capacity"])
    print("H2 Storage Capacity (tonnes): ", h2_storage_results["h2_capacity"]*1E-3)
    print("Electorlyzer CAPEX installed $/kW: ", capex_breakdown["electrolyzer"]/(plant_config["electrolyzer"]["rating"]*1E3))
    ...
    if len(solver_results) > 0:
        ...
        # print(annual_energy_usage)
        # power_df = pd.DataFrame.from_dict(annual_energy_usage)
        # power_df.to_csv("data/energy/energy_breakdown_case")
    
    # combine all the dicts

    # df_hopp = pd.DataFrame.from_dict(hopp_results)
    # df_electrolyzer = pd.DataFrame.from_dict(electrolyzer_physics_results)
    # df_desal = pd.DataFrame.from_dict(desal_results)
    # df_platform = pd.DataFrame.from_dict(platform_results)
    # df_h2_storage = pd.DataFrame.from_dict(h2_storage_results)
    # df_capex = pd.DataFrame.from_dict(capex_breakdown)
    # df_opex = pd.DataFrame.from_dict(opex_breakdown)
    del hopp_results["hybrid_plant"]
    del electrolyzer_physics_results["H2_Results"]
    # print(electrolyzer_physics_results.keys())
    # dicts = [plant_config, electrolyzer_physics_results, desal_results, platform_results, h2_storage_results, capex_breakdown, opex_breakdown]
    
    # output_dict = dict()
    # for d in dicts:
    #     output_dict = dict(output_dict, **d)

    # output_dict = dict()
    # output_dict["electrolyzer capacity factor"] = electrolyzer_physics_results["capacity_factor"]
    # output_dict[""]
    # filename = 'data/info/result_disign%i_policy%i.yml' %(plant_design_number, incentive_option)
    ...
    # for d in dicts:
    #     print(d)
    # df_output = pd.concat(dicts, axis=0, join="outer")

    # print(df_output)
    # print("LCOE ($/MWh): ", lcoe*1E3)
    # print("LCOH ($/kg): ", lcoh)
    # print("LCOH w/grid ($/kg): ", lcoh_with_grid)
    # print("LCOH grid only ($/kg): ", lcoh_grid_only)
    # save processed data
    ...
    # # wind_speed = [W[2] for W in wind_resource._data['data']]
    # # hourly
    # ax[0].plot([W[2] for W in hopp_results["hybrid_plant"].site.wind_resource._data['data']])
    # ax[0].set(ylabel="Wind (m/s)", yticks=range(0,26,5))
    # ax[1].plot((np.array(hopp_results["combined_pv_wind_power_production_hopp"])*1E-3))
    # ax[1].set(ylabel="Energy (MWh)", ylim=[0,800], yticks=range(0,801,200))
    # ax[2].plot(np.array(hopp_results["combined_pv_wind_curtailment_hopp"])*1E-3)
    # ax[2].set(ylabel="Cur. (MWh)", ylim=[0, 300])
    # ax[3].plot(electrolyzer_physics_results["H2_Results"]["hydrogen_hourly_production"])
    # ax[3].set(xlabel="Hour", ylabel="H2 (kg/hr)", yticks=range(0,15001,5000))

    # # ## Expenditures
    # # fig, ax = plt.subplots(4,2, sharex='col')
    # # # wind_speed = [W[2] for W in wind_resource._data['data']]
    # # # hourly
    # # ax[0,0].plot([W[2] for W in hopp_results["hybrid_plant"].site.wind_resource._data['data']])
    # # ax[0,0].set(ylabel="Wind (m/s)", yticks=range(0,26,5))
    # # ax[1,0].plot((np.array(hopp_results["combined_pv_wind_power_production_hopp"])*1E-3))
    # # ax[1,0].set(ylabel="Energy (MWh)", ylim=[0,800], yticks=range(0,801,200))
    # # ax[2,0].plot(np.array(hopp_results["combined_pv_wind_curtailment_hopp"])*1E-3)
    # # ax[2,0].set(ylabel="Cur. (MWh)")
    # # ax[3,0].plot(electrolyzer_physics_results["H2_Results"]["hydrogen_hourly_production"])
    # # ax[3,0].set(xlabel="Hour", ylabel="H2 (kg/hr)", yticks=range(0,15001,5000))
    # # # annually
    # # ex_df = pd.DataFrame(expenditures_breakdown)
    # # ex_df.index.name = "year"
    # # print(ex_df)
    # # ax[0,1].stackplot(ex_df.index, ex_df.T*1E-6)
    # # ax[0,1].set(ylabel="Cost ($M)")

    # # ax[0,1].stackplot(ex_df.index, ex_df.T*1E-9)
    # # ax[0,1].set(ylabel="Cost ($B)")
    # # # ax[0,1].legend()
    # # ax[3,1].scatter(np.arange(1,len(expenditures_annual)+1),-expenditures_annual*1E-9)
    # # ax[3,1].set(xlabel="Year", ylabel="Billion USD", xticks=np.arange(0,len(expenditures_annual)+1, 5), yticks=[0,1,2,3])
    # plt.tight_layout()
    # # plt.savefig("results-%s.pdf" %(plant_config["electrolyzer"]["rating"]), transparent=True)
    # plt.show()
...
def run_sweeps(simulate=False, verbose=True, show_plots=True, use_profast=True):
    ...
    if simulate:
        ...
        plant_sizes = [400] #[200, 400, 600, 800]

        for plant_size in plant_sizes:
            ratings = np.linspace(round(0.2*plant_size, ndigits=0), plant_size, 20)
    ...
    if show_plots:
        plant_sizes = [400, 400, 400]#[200, 400, 600, 800]
...
def run_policy_options_storage_types(verbose=True, show_plots=False, save_plots=False,  use_profast=True):
    ...
    np.savetxt("results/lcoh-with-policy.txt", np.c_[np.round(lcoh_array, decimals=2)], header="rows: %s, columns: %s" %("".join(storage_types), "".join(str(p) for p in policy_options)), fmt="%.2f")
    ...
    for i, design in enumerate(design_scenarios):
        for j, policy in enumerate(policy_options):
            for storage in storage_types:
                ...
                h2_production.append(electrolyzer_physics_results["H2_Results"]["hydrogen_annual_output"])
                ...
                annual_energy_breakdown_series["annual_h2_kg"].append(electrolyzer_physics_results["H2_Results"]["hydrogen_annual_output"])
                for key in annual_energy_breakdown.keys():
                    annual_energy_breakdown_series[key].append(annual_energy_breakdown[key])

    df = pd.DataFrame.from_dict({"Design": design_series, "Storage": storage_series, "Policy": policy_series,"LCOH [$/kg]": lcoh_series, "LCOE [$/kWh]": lcoe_series, "Electrolyzer capacity factor": electrolyzer_capacity_factor_series, "H2 Production [kg/yr]": h2_production})
    df.to_csv("data/design-storage-policy-lcoh.csv")

    df_energy = pd.DataFrame.from_dict(annual_energy_breakdown_series)
    df_energy.to_csv("data/annual_energy_breakdown.csv")
    return 0
...
def process_design_options(verbose=True, show_plots=False):
    ...
    ax = df_capex.plot.bar(x="Design", stacked=True, legend=True, ylabel="B USD", xlabel="Design Scenario", rot=0, title="CAPEX", ylim=[0,4], figsize=(8,5))
    ...
    print(df_capex)
    plt.savefig("figures/aggregate/bar-capex.pdf", transparent=True)
    plt.tight_layout()
    plt.show()

    ax = df_opex.plot.bar(x="Design", stacked=True, legend=True, ylabel="M USD", xlabel="Design Scenario", rot=0, title="OPEX", ylim=[0,100], figsize=(8,5))
    ax.legend(ncol=2, frameon=False, loc=2)
    ...
    # ax.set(xticks={"rotation": 90})
    print(df_opex)
    plt.savefig("figures/aggregate/bar-opex.pdf", transparent=True)
    plt.tight_layout()
    plt.show()

    df_metrics.plot.bar(x="Design", y="LCOH [$/kg]", legend=False, rot=0, ylabel="LCOH ($/kg)", xlabel="Design Scenario", figsize=(3,4))
    plt.tight_layout()
    plt.savefig("figures/aggregate/lcoh.png")
    plt.show()
    df_metrics.plot.bar(x="Design", y="LCOE [$/MWh]", legend=False, rot=0, ylabel="LCOE ($/MWh)", xlabel="Design Scenario", figsize=(3,4))
    plt.tight_layout()
