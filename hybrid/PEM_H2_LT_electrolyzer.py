...
class PEM_electrolyzer_LT:
    ...
    def compression_efficiency(self):
        ...
        W_C = s_C * W_1_C # Pressure-Volume work - energy reqd. for compression
    ...
    def h2_production_rate(self):
        ...
        h2_production_rate = n_Tot * ((self.N_cells * self.output_dict['current_input_external_Amps']) /
                                    (2 * self.F))  # mol/s
        ...
        print(np.sum(h2_produced_kg_hr_system))
        print(np.sum(self.input_dict['P_input_external_kW']))
        print(33.3 * np.sum(h2_produced_kg_hr_system)/np.sum(self.input_dict['P_input_external_kW']))
