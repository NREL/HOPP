...
policy_strings = {
                    'no-policy':'No incentives',
                    'base':'Base incentives',
                    'max':'Max incentives'
                    }
...
for axi1,policy in enumerate(policy_cases):
    for axi2,atb_year in enumerate(years):
        ...
        policy_year_electrolysis = financial_summary_electrolysis.loc[(financial_summary_electrolysis['Policy Option']==policy) & (financial_summary_electrolysis['Year']==atb_year)]
        policy_year_electrolysis['CCS Case'] = 'NA'
        policy_year_smr = financial_summary_smr.loc[(financial_summary_smr['Policy Option']==policy) & (financial_summary_smr['Year']==atb_year) & (financial_summary_smr['NG price case']=='default')]
