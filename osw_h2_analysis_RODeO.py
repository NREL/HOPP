    ...
    for atb_year in atb_years:
        for site_location in site_selection:
            for turbine_model in turbine_name:
                ...
                electrolyzer_capex_USD_per_MW = 1542000 # Eventually get from input loop
               ...
               # txt1 = '"C:\\GAMS\\win64\\24.8\\gams.exe" ..\\RODeO\\Storage_dispatch_SCS license=C:\\GAMS\\win64\\24.8\\gamslice.txt'
                ...
                efficiency = ' --input_efficiency_inst='+str(eta_LHV)#'0.611'#+str(round(math.sqrt(RTE[i1-1]),6))#+' --output_efficiency_inst='+str(round(math.sqrt(RTE[i1-1]),6))
                storage_cap = ' --storage_cap_instance=1000'#+str(stor_dur[i1-1])
                ...
                allow_import_inst = ' --allow_import_instance=0'
                input_LSL_inst = ' --input_LSL_instance=0'
                ren_capcost = ' --renew_cap_cost_inst='+str(hybrid_installed_cost_perMW)#'1230000'
                input_capcost= ' --input_cap_cost_inst='+str(round(electrolysis_desal_total_capex_per_MW))#'1542000'
                prodstor_capcost = ' --ProdStor_cap_cost_inst=26'
                ...
                RODeO_results_summary = pd.read_csv(dirout+'\\Storage_dispatch_summary_steel_'+str(atb_year)+'_' +site_location.replace(' ','_')+'_'+turbine_model + '.csv',header = 1,sep=',')
                ...
                hydrogen_hourly_inputs_RODeO = pd.read_csv(dirout+'\\Storage_dispatch_inputs_steel_'+str(atb_year)+'_' +site_location.replace(' ','_')+'_'+turbine_model + '.csv',index_col = None,header = 29)
                hydrogen_hourly_results_RODeO = pd.read_csv(dirout+'\\Storage_dispatch_results_steel_'+str(atb_year)+'_' +site_location.replace(' ','_')+'_'+turbine_model + '.csv',index_col = None,header = 26)
